/*****************************************************************
 * Code-Webapp.gs — InsureFlow backend (Google Apps Script Web App)
 * -------------------------------------------------------------
 * SINGLE merged file: CRM dashboard + writes + login + questionnaire intake.
 * Use this as the ONLY script file in the project (delete Code-Website.gs so
 * there is exactly one doGet and one doPost).
 *
 * GET  (no action)        → returns all data shaped as window.DATA (JSON/JSONP)
 * GET  ?action=addCustomer→ appends a row and returns the new record
 *      ?action=addLead     (writes use GET+JSONP so the static page can call
 *      ?action=addAgent     them cross-origin with no CORS setup)
 *      ?action=addNote
 * POST ?action=logQuestionnaire  (JSON body) → analytics row in the
 *      questionnaire's own tab, one column per question (from the website)
 *
 * DEPLOY / REDEPLOY:
 *   1. Spreadsheet → Extensions → Apps Script → paste this file → Save.
 *   2. Deploy → Manage deployments → (edit the existing Web app) →
 *      Version: New version → Deploy.  Keeping the SAME deployment keeps
 *      the /exec URL stable, so the website needs no change.
 *      Execute as: Me   ·   Who has access: Anyone
 *
 * NOTE: "Anyone" means the /exec URL can trigger writes. The URL lives in
 * client JS so treat it as a low-trust endpoint; do not store secrets here.
 *****************************************************************/
...
function doGet(e) {
  ...
  let out;
  try {
    if (action === 'login') out = login(e.parameter.user, e.parameter.pass);
    else if (action) out = handleWrite(action, e.parameter);
    else out = buildData();
  } catch (err) {
...
/* doPost — same router as doGet, plus questionnaire intake (JSON body). */
function doPost(e) {
  const p = (e && e.parameter) || {};
  let out;
  try {
    if (p.action === 'login') out = login(p.user, p.pass);
    else if (p.action === 'logQuestionnaire') out = logQuestionnaire_(e); // website analytics
    else if (p.action) out = handleWrite(p.action, p);
    else out = { error: 'Missing action' };
  } catch (err) {
...
/* ===================== Questionnaire intake (website) =====================
   The website POSTs to <exec>?action=logQuestionnaire with a JSON body:
     { source, event, score, segment, name, email, phone, summary, pageUrl,
       fields: { questionId: answerLabel, ... } }
   Each questionnaire gets its own tab; each question becomes its own column
   (added automatically the first time it appears). Leads are still created by
   the addLead action (the website fires both). */
const Q_HEADERS = ['Timestamp', 'Event', 'Score', 'Segment', 'Name', 'Email', 'Phone', 'Summary', 'Page'];

function qTabNameFor_(source) {
  switch (source) {
...
function logQuestionnaire_(e) {
  var data = {};
  try { if (e && e.postData && e.postData.contents) data = JSON.parse(e.postData.contents); } catch (err) {}
  qLogSourceTab_(data);
  return { ok: true };
}

function qLogSourceTab_(data) {
  if (!data || (!data.source && !data.fields && !data.event)) return;
  var ss = ss_();
  var name = qTabNameFor_(data.source);
  var sheet = ss.getSheetByName(name);
  if (!sheet) { sheet = ss.insertSheet(name); sheet.appendRow(Q_HEADERS); sheet.setFrozenRows(1); }

  var headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0].map(String);
  var fields = data.fields || {};
  var missing = Object.keys(fields).filter(function (k) { return headers.indexOf(k) === -1; });
  if (missing.length) {
    sheet.getRange(1, headers.length + 1, 1, missing.length).setValues([missing]);
    headers = headers.concat(missing);
  }

  var contact = String(data.contact || ''), isEmail = contact.indexOf('@') > -1;
  var email = String(data.email || (isEmail ? contact : ''));
  var phone = String(data.phone || (isEmail ? '' : contact));
  var row = {
    Timestamp: data.submittedAt || nowIso(), Event: data.event || 'completion',
    Score: (data.score === 0 || data.score) ? data.score : '', Segment: data.segment || '',
    Name: data.name || '', Email: email, Phone: phone,
    Summary: data.summary || '', Page: data.pageUrl || '',
  };
...
function buildData() {
  ...
  return {
    ...
    PULSE: pulse,
    MONTHLY: monthly,
    _meta: { generatedAt: Utilities.formatDate(NOW, TZ, "yyyy-MM-dd'T'HH:mm:ssXXX"), source: 'google-sheets' },
  };
}
...
function custNext(bi, renewalDays, renewalDate, followDays, followDate) {
  const c = [];
  if (bi.days != null && bi.days >= 0)
    c.push({ d: bi.days, label: bi.days === 0 ? 'Birthday · today' : 'Birthday · ' + bi.label });
  if (renewalDays != null && renewalDays >= 0)
    c.push({ d: renewalDays, label: 'Renewal · ' + fmtShort(renewalDate) });
  if (followDays != null && followDays >= 0)
    c.push({ d: followDays, label: followDays === 0 ? 'Follow-up · today' : 'Follow-up · ' + fmtShort(followDate) });
...
function mapLeadStage(s) {
  ...
  if (s.indexOf('won') >= 0 || s.indexOf('clos') >= 0 || s.indexOf('convert') >= 0 ||
      s.indexOf('sign') >= 0 || s.indexOf('lost') >= 0) return 'closed';
  return 'new';
}
...
function buildAgents(rows) {
  return rows.map(function (a) {
    ...
    let date;
    if (a.InterviewDate)
      date = 'Interview · ' + fmtShort(a.InterviewDate) + (a.InterviewTime ? ' ' + fmtTime(a.InterviewTime) : '');
    else if (a.LicenseDate && stage === 'licensed')
      date = 'Licensed · ' + fmtShort(a.LicenseDate);
    else if (a.NextFollowUpDate)
      date = 'Follow-up · ' + fmtShort(a.NextFollowUpDate);
    else
      date = relativeFrom(a.UpdatedAt || a.CreatedAt) || '—';
    return {
...
function buildMonthly(customers, leads) {
  ...
  return {
    premium: compactBaht(premium), delta: '', vs: 'in force',
...
function buildCeoFocus(customers, leads, tasks) {
  ...
  return {
    ...
    insight: 'Focus on customer retention today — ' + renewals7 + ' renewal' + (renewals7 === 1 ? '' : 's') +
             ' are due this week. ' + (leads.filter(function (l) { return l.stage === 'new'; }).length) +
             ' new leads remain uncontacted.',
  };
}
...
function buildUpcoming(custRows, agentRows) {
  ...
  custRows.forEach(function (c) {
    const bi = bdayInfo(c.BirthdayMMDD, c.BirthDate);
    if (bi.days != null && bi.days >= 0 && bi.days <= 30)
      ev.push({ d: bi.days, date: bi.date, tone: 'warn', kind: 'Birthday', name: nameOf(c), icon: 'gift', bday: true });
    const rd = c.NearestRenewalDate ? daysFromNow(c.NearestRenewalDate) : null;
    if (rd != null && rd >= 0 && rd <= 45)
      ev.push({ d: rd, date: toDate(c.NearestRenewalDate), tone: 'due', kind: 'Renewal', name: nameOf(c), icon: 'renew' });
    const fd = c.NextFollowUpDate ? daysFromNow(c.NextFollowUpDate) : null;
    if (fd != null && fd >= 0 && fd <= 30)
      ev.push({ d: fd, date: toDate(c.NextFollowUpDate), tone: 'ok', kind: 'Check-in', name: nameOf(c), icon: 'phone' });
  });
  agentRows.forEach(function (a) {
    const id = a.InterviewDate ? daysFromNow(a.InterviewDate) : null;
    if (id != null && id >= 0 && id <= 30)
      ev.push({ d: id, date: toDate(a.InterviewDate), tone: 'info', kind: 'Interview', name: nameOf(a), icon: 'user' });
  });
...
function sumPremium(customers) {
  return customers.reduce(function (s, c) { return s + parseCompact(c.premium); }, 0);
}
...
function bdayInfo(mmdd, birth) {
  ...
  if (mmdd && String(mmdd).indexOf('-') >= 0) {
    ...
    const b = toDate(birth); if (!b) return { days: null, label: '', date: null };
    mm = b.getMonth() + 1; dd = b.getDate();
  }
  if (!mm || !dd) return { days: null, label: '', date: null };
  let next = new Date(NOW.getFullYear(), mm - 1, dd);
  if (Math.round((startOfDay(next) - startOfDay(NOW)) / 86400000) < 0)
...
function tseconds(v) {
  ...
  return m ? (+m[1] * 3600 + +m[2] * 60) * 1000 : 0;
}
